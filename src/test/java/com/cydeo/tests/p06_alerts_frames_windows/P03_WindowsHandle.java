package com.cydeo.tests.p06_alerts_frames_windows;

import com.microsoft.playwright.Browser;
import com.microsoft.playwright.BrowserType;
import com.microsoft.playwright.Page;
import com.microsoft.playwright.Playwright;
import org.junit.jupiter.api.*;
import com.cydeo.utils.BrowserUtils;

public class P03_WindowsHandle {

    static private Playwright playwright;

    static private Browser browser;

    private Page page;

    @BeforeAll
    static void beforeAll() {
        playwright = Playwright.create();
        browser = playwright.chromium().launch(new BrowserType.LaunchOptions()
                .setHeadless(false)
//                .setSlowMo(1000) // Do not use windows handle or use promise.
        );
    }


    @AfterAll
    static void afterAll() {

        browser.close();
        playwright.close();
    }


    @BeforeEach
    void setUp() {
        page = browser.newContext().newPage();
        page.navigate("https://practice.cydeo.com/windows");
    }

    @AfterEach
    void tearDown() {
        page.close();
    }

    @Test
    void test1() {

        page.pause(); // pause() is for debugging.
        //get the title
        System.out.println("before clicking page title = " + page.title());

        //click "Click Here" button (it will open a new tab)
        BrowserUtils.sleepWithThread(3);
//        page.click("text=Click Here"); //It's wrong.
        Page secondPage = page.waitForPopup(() -> {
            page.getByText("Click Here").click();
        }); // This method is coming from JavaScript.

//        Page secondPage = page.waitForPopup(() -> {}); //todo: it's one challenge. it's about line 23 slow motion.

        //get second window title
        System.out.println("after clicking button second page title = " + secondPage.title());

        BrowserUtils.sleepWithThread(3);

        //go back to first page
        page.bringToFront();
        BrowserUtils.sleepWithThread(3);
        secondPage.bringToFront();
        BrowserUtils.sleepWithThread(3);

        page.bringToFront();

        //Click home
        page.click("text=Home");

        BrowserUtils.sleepWithThread(3);

        //get the title again
        System.out.println("practice title = " + page.title());

        secondPage.close();

    }
}
